            ...
            for ($i = 0 ; $i < sizeof($arrayComment) ; ++$i)
            {
                ...
                if (isset($_SESSION['suid'])){
                    if ($_SESSION['user']->getUserId() === $billetClique->getAuthorId()){
                        if ($isImportante === 0){
                    ?>
                            <button class="btnupvote" form="CommentAction" name="makeImportante" value="<?php echo $arrayComment[$i]->getCommentId()?>"> up</button>
                    <?php
                        }
                        else {
                    ?>
                            <button class="btndownvote" form="CommentAction" name="unMakeImportante" value="<?php echo $arrayComment[$i]->getCommentId()?>">Down</button>
                    <?php
                        }
                    }
                ?>
                <?php
                    if ($_SESSION['user']->getIsAdmin() || $_SESSION['user']->getUserId()===$arrayComment[$i]->getAuthor()) {?>
                        <button class="btnpoubelle" form="CommentAction" name="DelComment" value="<?php echo $arrayComment[$i]->getCommentId()?>">Supprimer</button>
                        <?php
                        if ($_SESSION['user']->getUserId()===$arrayComment[$i]->getAuthor()) {
                            if(isset($_POST['CommentModifEtat'])){?>
                                <button class="btnpoubelle" form="CommentAction" title="Ecriver dans la zone de texte puis cliquez pour modifier" name="CommentModifier" value="<?php echo $arrayComment[$i]->getCommentId()?>">Valider</button>
                                <button class="btnpoubelle" form="CommentModif" name="CommentModifierCancel">Annuler</button>
                            <?php
                            }
                            else{ ?>
                            <button class="btnmodif" form="CommentModif" name="CommentModifEtat" value="<?php echo $arrayComment[$i]->getCommentId()?>">modification</button>
                            <?php
                            }
                        }
                    }
