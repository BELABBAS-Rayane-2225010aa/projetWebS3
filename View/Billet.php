...
<script src="Bouton.js"></script>
    <button onclick="BoutonAffichageCategorie()" class="buttonCategory"><i class=""></i></button><!--Présent qu'en mode portable sert a ameliorer la lisibiliter du billet-->
    ...
    if(isset($_SESSION['suid'])) { ?>
        <form action="../index.php" method="post" class="Comments">
            ...
            <input class="btnCommentCategory" type="submit" name="addComment" <span class=""> </span>
            ...
            for ($i = 0 ; $i < sizeof($arrayComment) ; ++$i)
            {
                ...
                if (isset($_SESSION['suid'])){
                    if ($_SESSION['user']->getUserId() === $billetClique->getAuthorId()){
                        if ($isImportante === 0){
                    ?>
                            <button class="btnupvote" form="CommentAction" name="makeImportante" value="<?php echo $arrayComment[$i]->getCommentId()?>"> <i class="material-icons">arrow_upward</i></button>
                    <?php
                        }
                        else {
                    ?>
                            <button class="btndownvote" form="CommentAction" name="unMakeImportante" value="<?php echo $arrayComment[$i]->getCommentId()?>"><i class=""></i></button>
                    <?php
                        }
                    }
                ?>
                <?php
                    if ($_SESSION['user']->getIsAdmin() || $_SESSION['user']->getUserId()===$arrayComment[$i]->getAuthor()) {?>
                        <button class="btnpoubelle" form="CommentAction" name="DelComment" value="<?php echo $arrayComment[$i]->getCommentId()?>"><i class=""></i></button>
                        <?php
                        if ($_SESSION['user']->getUserId()===$arrayComment[$i]->getAuthor()) {
                            if(isset($_POST['CommentModifEtat'])){?>
                                <button class="btnpoubelle" form="CommentAction" title="Ecriver dans la zone de texte puis cliquez pour modifier" name="CommentModifier" value="<?php echo $arrayComment[$i]->getCommentId()?>"><i class=""></i></button>
                                <button class="btnpoubelle" form="CommentModif" name="CommentModifierCancel"><i class=""></i></button>
                            <?php
                            }
                            else{ ?>
                            <button class="btnmodif" form="CommentModif" name="CommentModifEtat" value="<?php echo $arrayComment[$i]->getCommentId()?>"><i class=""></i></button>
                            <?php
                            }
                        }
                    }
